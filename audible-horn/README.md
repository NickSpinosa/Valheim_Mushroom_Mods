...
The horn recording is **"Hunting Horn.wav" by Benboncan**, from
[Freesound](https://freesound.org/people/Benboncan/sounds/72753/), used under the
[Creative Commons Attribution 4.0 International licence](https://creativecommons.org/licenses/by/4.0/)
(CC BY 4.0). It is mixed to mono, trimmed, faded and normalised before it is
embedded. Full attribution and the exact changes are in
[CREDITS.md](CREDITS.md).

**The clip currently shipping is still the placeholder, not that recording.** It
is a synthesised tone — a 2.5 s additive sine at 220 Hz with two harmonics —
generated by
[`tools/make-placeholder-horn.ps1`](tools/make-placeholder-horn.ps1), which is
committed so the file is reproducible, and it is meant to sound obviously
synthetic so nobody mistakes it for the final asset. The recording above is
approved and pending conversion; the command that swaps it in is at the bottom of
[CREDITS.md](CREDITS.md), and
[`tools/convert-horn.ps1`](tools/convert-horn.ps1) does the conversion in pure
.NET, without ffmpeg.

Everything else in this mod is original. The code is MIT; the recording keeps its
own licence.
