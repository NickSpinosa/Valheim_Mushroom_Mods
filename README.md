...
| [Mushroom Sync](MushroomSync/README.md) | No gameplay of its own — shared server-authoritative sync, required by five of the mods above |
...
└── BepInEx/
    └── plugins/
        ...
        ├── HaldorExpansion.dll
        ├── MushroomSync.dll          <- required by five of the above
        ├── RandomYggdrasil.dll
        ├── SeparateSpawns.dll
        └── VegvisirCompass.dll
```
...
Keep `MushroomSync.dll` if you keep Audible Horn, Combat Adjustments, Craftable
Spawners, Haldor Expansion or Random Yggdrasil — they depend on it and will not
load without it. Separate Spawns and Vegvísir Compass are independent.
