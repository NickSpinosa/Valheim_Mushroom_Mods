...
- Logging: only `Craftable spawners initialized.` and the per-join `Registered N spawner prefab(s) with ZNetScene.` are unconditional. Placement, hammer-removal, table additions and above all the per-attempt `SpawnOne` line go through `Dbgl` and need `Troubleshooting.EnableDebugMessages`. They were left at Info under a `[DEBUG-unlock]` prefix from the unlock work; one nest wrote a line every ten seconds, and 823 of them were in the client log that diagnosed the rejoin bug. The `SpawnOne` postfix checks the setting before anything else because it sees every vanilla spawner as well.
- `No Hammer PieceTable found` used to warn on every return to the main menu. The start scene's `ObjectDB` runs `Awake` with an empty item list and is filled by `CopyOtherDB` afterwards, so there was no Hammer to look at yet. The warning is now reserved for a Hammer that exists and has no table.
